...
import { actionCreateMetadata, actionUpdateMetadata, actionDeleteMetadata } from "./actions"
...
export default function MetadataContent({ rows }: { rows: Row[] }) {
  const { t } = useLanguage()
  ...
  // Lock page scroll while the form is open so the overlay always covers the viewport.
  useEffect(() => {
  ...
  function openNew() {
    setEditing(null); setForm({ label: "", value: "" }); setError(""); setShowForm(true)
  }
  function openEdit(r: Row) {
    setEditing(r); setForm({ label: r.label, value: r.value }); setError(""); setShowForm(true)
  }
  function closeForm() {
    setShowForm(false); setEditing(null); setForm({ label: "", value: "" }); setError("")
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) {
        await actionUpdateMetadata(editing.id, fd)
        setList(list.map(r => r.id === editing.id ? { ...r, label: form.label, value: form.value } : r))
        closeForm()
      } else {
        await actionCreateMetadata(fd)
  ...
  async function handleDelete(r: Row) {
    try {
      await actionDeleteMetadata(r.id)
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <h1 className="text-3xl font-bold text-gray-900 tracking-wide">{tr.metadataTitle}</h1>
        </div>
        <button
          onClick={openNew}
        ...
        <div className="fixed inset-0 bg-black/70 z-100 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            <h2 className="text-base font-bold text-gray-900 uppercase tracking-wide mb-5">
              {editing ? tr.metadataFormEdit : tr.metadataFormNew}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className={labelClass}>{tr.metadataLabelField}</label>
                <input
                  className={inputClass}
                  placeholder={tr.metadataLabelPlaceholder}
                  value={form.label}
              ...
              <div>
                <label className={labelClass}>{tr.metadataValueField}</label>
                <input
                  className={inputClass}
                  placeholder={tr.metadataValuePlaceholder}
                  value={form.value}
        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b border-gray-200">
              <th className="px-6 py-4 text-left text-[10px] font-bold text-gray-500 tracking-widest uppercase">{tr.metadataColLabel}</th>
              <th className="px-6 py-4 text-left text-[10px] font-bold text-gray-500 tracking-widest uppercase">{tr.metadataColValue}</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {list.map(r => (
              <tr key={r.id} className="hover:bg-gray-50 transition">
                <td className="px-6 py-4 text-sm font-bold text-gray-900">{r.label}</td>
                <td className="px-6 py-4 text-sm text-gray-700">{r.value}</td>
            ...
            {list.length === 0 && (
              <tr><td colSpan={3} className="px-6 py-12 text-center text-gray-400 text-sm">{tr.metadataEmpty}</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <ConfirmModal
        ...
        message={tr.metadataDeleteConfirm}
